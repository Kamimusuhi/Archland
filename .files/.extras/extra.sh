...
show_menu() {
	whiptail --title "Additional Configuration" --checklist --separate-output "Select the apps you want to download:" 20 60 14 \
		...
		"Hyprlock" "" on \
		"Z Shell" "" on "Nicotine+ (Music-sharing Client)" "" off \
		"Signal Desktop" "" on \
...
download_apps() {
	while IFS= read -r app; do
		...
		"VS-Code (Microsoft)")
			...
			code --install-extension cofeek-codes.catppuccin-dark-fixed
			code --install-extension esbenp.prettier-vscode
			code --install-extension vscodevim.vim
			sudo rm $HOME/.config/Code/User/settings.json
			sudo rm $HOME/.config/Code/User/keybindings.json
      cp -r $HOME/Archland/.files/.extras/Code/User/* $HOME/.config/Code/User/ 
			;;
		"VS-Codium")
